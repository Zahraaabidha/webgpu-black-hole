...
export class BlackHoleSimulation {
  ...
  initializeUniforms(config) {
    this.uniforms = {
      ...
      // Vertical (Y-axis) thickness of the disk slab and how sharply density
      // falls off away from the midplane. Previously defined in config but
      // never connected to the shader.
      diskThickness: uniform(config.diskThickness ?? 1.0),
      heightDensityFalloff: uniform(config.heightDensityFalloff ?? 5.0),
  ...
  updateUniforms(config) {
    ...
    if (config.diskThickness !== undefined) u.diskThickness.value = config.diskThickness;
    if (config.heightDensityFalloff !== undefined) u.heightDensityFalloff.value = config.heightDensityFalloff;
