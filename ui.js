...
export class BlackHoleUI {
  ...
  setupBlackHoleFolder() {
    ...
    geometryFolder.addBinding(this.config, 'diskThickness', {
      min: 0.1,
      max: 4.0,
      step: 0.05,
      label: 'Thickness'
    }).on('change', () => {
      this.callbacks.onUniformChange('diskThickness', this.config.diskThickness);
    });

    geometryFolder.addBinding(this.config, 'heightDensityFalloff', {
      min: 0.5,
      max: 10.0,
      step: 0.1,
      label: 'Edge Sharpness (Y)'
    }).on('change', () => {
      this.callbacks.onUniformChange('heightDensityFalloff', this.config.heightDensityFalloff);
    });
  ...
  // ==========================================================================
  // POST FX (vignette, film grain, chromatic aberration)
  // ==========================================================================

  setupPostFXFolder() {
    const fxFolder = this.pane.addFolder({ title: 'Cinematic FX', expanded: true });

    fxFolder.addBinding(this.config, 'chromaticAberration', {
      min: 0,
      max: 2,
      step: 0.01,
      label: 'Chromatic Aberration'
    }).on('change', () => {
      this.callbacks.onPostFXChange('chromaticAberration', this.config.chromaticAberration);
    });

    const vignetteFolder = fxFolder.addFolder({ title: 'Vignette', expanded: true });

    vignetteFolder.addBinding(this.config, 'vignetteStrength', {
      min: 0,
      max: 1,
      step: 0.01,
      label: 'Strength'
    }).on('change', () => {
      this.callbacks.onPostFXChange('vignetteStrength', this.config.vignetteStrength);
    });

    vignetteFolder.addBinding(this.config, 'vignetteRadius', {
      min: 0,
      max: 1.2,
      step: 0.01,
      label: 'Radius'
    }).on('change', () => {
      this.callbacks.onPostFXChange('vignetteRadius', this.config.vignetteRadius);
    });

    vignetteFolder.addBinding(this.config, 'vignetteSoftness', {
      min: 0.01,
      max: 1.2,
      step: 0.01,
      label: 'Softness'
    }).on('change', () => {
      this.callbacks.onPostFXChange('vignetteSoftness', this.config.vignetteSoftness);
    });

    const grainFolder = fxFolder.addFolder({ title: 'Film Grain', expanded: true });

    grainFolder.addBinding(this.config, 'filmGrainAmount', {
      min: 0,
      max: 0.3,
      step: 0.005,
      label: 'Amount'
    }).on('change', () => {
      this.callbacks.onPostFXChange('filmGrainAmount', this.config.filmGrainAmount);
    });

    grainFolder.addBinding(this.config, 'filmGrainSize', {
      min: 0.2,
      max: 5,
      step: 0.1,
      label: 'Grain Size'
    }).on('change', () => {
      this.callbacks.onPostFXChange('filmGrainSize', this.config.filmGrainSize);
    });
  }
  ...
  setupMusicFolder() {
    const musicFolder = this.pane.addFolder({ title: 'Music', expanded: true });

    this.musicState = { muted: this.config.musicMuted ?? false };

    this.muteButton = musicFolder.addButton({
      title: this.musicState.muted ? 'Unmute' : 'Mute'
    }).on('click', () => {
      const muted = this.callbacks.onMusicToggleMute?.();
      this.musicState.muted = muted;
      this.muteButton.title = muted ? 'Unmute' : 'Mute';
    });

    musicFolder.addBinding(this.config, 'musicVolume', {
      min: 0,
      max: 1,
      step: 0.01,
      label: 'Volume'
    }).on('change', () => {
      this.callbacks.onMusicVolumeChange?.(this.config.musicVolume);
    });
