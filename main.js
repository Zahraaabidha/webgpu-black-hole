...
// NOTE: This config was previously cluttered with ~40 keys (ringScale,
// qualityPreset, temporalAA, duplicate nebulaColor1/2 systems, etc.) that
// were never read anywhere in blackhole.js or blackhole-shader.js — leftover
// from an abandoned feature branch. Trimmed to only what's actually wired up.
// `diskThickness` / `heightDensityFalloff` are now live (see blackhole-shader.js).
const defaultConfig = {
  "blackHoleMass": 0.4,
  "diskInnerRadius": 4.1,
  ...
  "chromaticAberration": 0.61,
  "filmGrainAmount": 0.00,
  "filmGrainSize": 0.2,
...
const backgroundMusic = new BackgroundMusic({
  sources: ['./audio/theme.mp3'],
  volume: config.musicVolume,
  loop: true
});
backgroundMusic.muted = config.musicMuted;
backgroundMusic.armAutoplayUnlock();
...
const ui = new BlackHoleUI(config, {
  ...
  // Handle post-FX (vignette/grain/chromatic aberration) changes
  onPostFXChange: (key, value) => {
    if (postFX[key]) {
      postFX[key].value = value;
    }
  },
