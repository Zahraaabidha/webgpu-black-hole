...
const createAccretionDiskColor = (uniforms) => Fn(([hitR, hitAngle, time, rayDir, heightFrac]) => {
  ...
  const finalOpacity = ringOpacity.mul(edgeFalloff).mul(heightFrac);
...
export function createBlackHoleShader(uniforms) {
  ...
  return Fn(() => {
    ...
    Loop(64, () => {
      ...
      // Disk plane intersection (Y = 0), with finite thickness
      ...
      If(crossedPlane.and(alpha.lessThan(0.99)), () => {
        ...
        If(inDisk, () => {
          ...
          // Approximate vertical thickness: instead of an infinitely thin plane,
          // treat the segment that crosses Y=0 as passing through a slab of
          // half-thickness `diskThickness`, and weight opacity by how much of
          // the step's path length actually sits inside that slab. This gives
          // the disk visible "puffiness" without a full volumetric march.
          const segLen = length(rayPos.sub(prevPos)).max(0.0001);
          const halfThickness = uniforms.diskThickness.mul(0.5).max(0.001);
          const slabFraction = clamp(halfThickness.mul(2.0).div(segLen.mul(abs(rayDir.y).max(0.05))), float(0.05), float(1.0));
          const heightFrac = pow(slabFraction, uniforms.heightDensityFalloff.mul(0.1).add(0.3));

          const diskResult = accretionDiskColor(hitR, hitAngle, uniforms.time, rayDir, heightFrac);
